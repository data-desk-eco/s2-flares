...
pub fn detect_candidates(
    cands: &[Candidate],
    overview_cloud_free: bool,
    t: &Thresholds,
) -> (Vec<Detection>, bool) {
    let mut all = Vec::new();
    let mut cloud_free = overview_cloud_free;
    for c in cands {
        let (dets, cf) = detect_block(
            &c.b12,
            &c.b11,
            c.b8a.as_deref(),
            c.scl.as_deref(),
        ...
        for d in dets {
            if (d.peak_img_row as usize) / BLOCK_SIZE == c.br
                && (d.peak_img_col as usize) / BLOCK_SIZE == c.bc
            {
                all.push(d);
            }
        }
...
pub(crate) struct Radiometry {
    pub offset: f64,
    pub quantification: f64,
}

pub(crate) fn b12_dn_min(t: &Thresholds, r: Radiometry) -> f64 {
    t.b12_min * r.quantification - r.offset
}

fn tag_number(xml: &str, tag: &str) -> Option<f64> {
    let start = xml.find(&format!("<{tag}"))?;
    let value = &xml[start..];
    let gt = value.find('>')? + 1;
    let end = value[gt..].find('<')? + gt;
    value[gt..end].trim().parse().ok()
}

pub(crate) fn vsi_read(url: &str) -> Result<Vec<u8>, String> {
    use std::ffi::CString;
    let path = CString::new(to_vsi(url)).map_err(|e| e.to_string())?;
    let mode = CString::new("rb").unwrap();
    unsafe {
        let file = gdal_sys::VSIFOpenL(path.as_ptr(), mode.as_ptr());
        if file.is_null() {
            return Err(format!("open metadata {url}"));
        }
        gdal_sys::VSIFSeekL(file, 0, 2);
        let len = gdal_sys::VSIFTellL(file) as usize;
        gdal_sys::VSIFSeekL(file, 0, 0);
        let mut bytes = vec![0u8; len];
        let n = gdal_sys::VSIFReadL(bytes.as_mut_ptr().cast(), 1, len, file);
        gdal_sys::VSIFCloseL(file);
        if n != len {
            return Err(format!("short metadata read {url}: {n}/{len}"));
        }
        Ok(bytes)
    }
}

/// Read the SAFE-declared radiometry. The date fallback covers public COGs that do
/// not expose product metadata, while CDSE JP2 runs always take the authoritative XML.
pub(crate) fn radiometry(item: &Item) -> Result<Radiometry, String> {
    let baseline_offset = if item.date.as_str() >= "2022-01-25" {
        -1000.0
    } else {
        0.0
    };
    if let Some(url) = &item.bands.product_metadata {
        let bytes = vsi_read(url)?;
        let xml = String::from_utf8_lossy(&bytes);
        let offset = tag_number(
            &xml,
            if item.level == "l1c" {
                "RADIO_ADD_OFFSET"
            } else {
        ...
        // AWS granule metadata before newer processing baselines omits this
        // product-level element. The collection baseline transition is still
        // authoritative, so absence is not the same thing as an offset of zero.
        .unwrap_or(baseline_offset);
        let quantification = tag_number(
            &xml,
            if item.level == "l1c" {
...
pub fn configure() {
    let endpoint = std::env::var("AWS_S3_ENDPOINT")
        .unwrap_or_else(|_| "eodata.dataspace.copernicus.eu".into());
    for (k, v) in [
...
pub(crate) fn open(href: &str) -> Result<Raster, String> {
    ...
    let gt = ds
        .geo_transform()
        .map_err(|e| format!("geotransform: {e}"))?;
...
pub(crate) fn read_window<T: Copy + gdal::raster::GdalType>(
    r: &Raster,
    win: [usize; 4],
) -> Option<Vec<T>> {
    let [x0, y0, x1, y1] = win;
    ...
    let buf = band
        .read_as::<T>((x0 as isize, y0 as isize), (w, h), (w, h), None)
        .ok()?;
    Some(buf.data().to_vec())
}

pub(crate) fn read_overview<T: Copy + gdal::raster::GdalType>(
    r: &Raster,
    utm_bbox: [f64; 4],
    factor: usize,
) -> Option<Vec<T>> {
    ...
    let y1 = (((max_y - utm_bbox[1]) / r.res_y)
        .ceil()
        .min(r.height as f64)) as usize;
...
pub(crate) fn any_hot(a: &[u16], floor: f64) -> bool {
    a.iter().any(|&v| v as f64 >= floor)
}
...
pub(crate) fn overview_screen(
    ...
    if let Some(ov) = read_overview::<u16>(b12, utm_bbox, OVERVIEW_FACTOR) {
        if !any_hot(&ov, b12_dn_min(t, radiometry)) {
            return Screen::Cold(cloud_free);
        }
    }
...
pub(crate) fn aux_open(b: &crate::stac::Bands, b11_url: &str) -> Result<Aux, String> {
    Ok((
        open(b11_url)?,
        b.b8a.as_ref().and_then(|u| open(u).ok()),
        b.scl.as_ref().and_then(|u| open(u).ok()),
    ))
}
...
pub(crate) fn block_meta(
    ...
    BlockMeta {
        ...
        radiometric_offset: radiometry.offset,
        quantification: radiometry.quantification,
    }
}
...
pub(crate) fn make_candidate(
    ...
    let b11 = read_window::<u16>(b11_r, blk.window)?;
    let b8a = b8a_r
        .as_ref()
        .and_then(|r| read_window::<u16>(r, blk.window));
    let scl = scl_r
        .as_ref()
        .and_then(|r| read_window::<u8>(r, blk.window));
    Some(Candidate {
        meta: block_meta(blk, item, geom, radiometry),
        br: blk.br,
        bc: blk.bc,
        b12,
        b11,
...
fn slice<T: Copy>(full: &[T], tile_w: usize, win: [usize; 4]) -> Vec<T> {
    ...
    for row in y0..y1 {
        let base = row * tile_w;
        out.extend_from_slice(&full[base + x0..base + x1]);
    }
    out
}
...
/// Radiometry comes from each product's SAFE metadata, so the reader is source-agnostic.
...
impl SceneReader for GdalReader {
    fn candidates(
        ...
        let utm_bbox = if full_tile {
            b12.bbox
        } else {
            region_utm_bbox(&b12, bbox, item.epsg)
        };

        let overview_cloud_free = match if screen_overview {
            overview_screen(&b12, b.scl.as_deref(), utm_bbox, t, calibration)
        } else {
            Screen::Go(true)
        } {
        ...
        let blocks = if full_tile {
            all_blocks(b12.width, b12.height)
        } else {
            enumerate_blocks(
                b12.width, b12.height, b12.bbox, b12.res_x, b12.res_y, bbox, item.epsg,
            )
        };
        let mut aux: Option<Aux> = None;
        let mut cands = Vec::new();
        let floor = b12_dn_min(t, calibration);

        for blk in &blocks {
            let b12_raw = match read_window::<u16>(&b12, blk.window) {
                Some(v) => v,
                None => continue,
            };
            if !any_hot(&b12_raw, floor) {
                continue;
            }
            if aux.is_none() {
                aux = Some(aux_open(b, b11_url)?);
            }
            if let Some(c) =
                make_candidate(blk, b12_raw, aux.as_ref().unwrap(), item, geom, calibration)
            {
                cands.push(c);
            }
        }
...
impl SceneReader for BulkReader {
    fn candidates(
        ...
        let (full_b12, full_b11, full_b8a, full_scl);
        if self.gpu {
            #[cfg(feature = "gpu")]
            {
                let (f12, f11, f8a) = crate::gpu::decode_bands(b12_url, b11_url, b.b8a.as_deref())?;
                if f12.len() != w * h || f11.len() != w * h {
                    return Err("gpu/gdal dim mismatch".into());
                }
                full_b12 = f12;
                full_b11 = f11;
                full_b8a = f8a;
                full_scl = b
                    .scl
                    .as_deref()
                    .and_then(|u| open(u).ok())
                    .and_then(|r| whole::<u8>(&r));
            }
            #[cfg(not(feature = "gpu"))]
            {
        ...
        let blocks = if full_tile {
            ...
            enumerate_blocks(w, h, b12.bbox, b12.res_x, b12.res_y, bbox, item.epsg)
        };
        let floor = b12_dn_min(t, calibration);
        let mut cands = Vec::new();
        for blk in &blocks {
            let b12s = slice(&full_b12, w, blk.window);
            if !any_hot(&b12s, floor) {
                continue;
            }
            cands.push(Candidate {
                meta: block_meta(blk, item, geom, calibration),
                br: blk.br,
                bc: blk.bc,
                b12: b12s,
                b11: slice(&full_b11, w, blk.window),
...
pub fn make_reader(gpu: bool, bulk: bool) -> Result<Box<dyn SceneReader>, String> {
    if gpu && !cfg!(feature = "gpu") {
        return Err("--gpu needs a --features gpu build (CUDA box)".into());
    }
    if gpu && !bulk {
        return Err("--gpu is the bulk full-tile path — use it with --region".into());
    }
    Ok(if bulk {
        Box::new(BulkReader { gpu })
    } else {
        Box::new(GdalReader)
    })
}
...
pub fn cloud_scene(item: &Item, bbox: [f64; 4], full_tile: bool) -> Vec<(String, f64)> {
    ...
    let blocks = if full_tile {
        all_blocks(r.width, r.height)
    } else {
        enumerate_blocks(r.width, r.height, r.bbox, r.res_x, r.res_y, bbox, item.epsg)
    };
    if blocks.is_empty() {
        return Vec::new();
    }
    ...
    for b in &blocks {
        x0 = x0.min(b.window[0]);
        y0 = y0.min(b.window[1]);
        x1 = x1.max(b.window[2]);
    ...
    cover_sites(&scl, r.width, r.height, r.bbox, item.epsg, &grid_sites(wb))
        .into_iter()
        .map(|c| {
            let f = c.cloud_frac();
            (c.h3, f)
...
pub fn detect_scene(
    ...
    t: &Thresholds,
    screen_overview: bool,
) -> Result<(Vec<Detection>, bool), String> {
...
mod parity_cpu {
    ...
    fn bulk_matches_windowed() {
        ...
        let (start, end) = (
            std::env::var("S2_PARITY_START").unwrap_or_else(|_| "2024-01-01".into()),
        ...
        let (t, win, bulk) = (Thresholds::default(), GdalReader, BulkReader { gpu: false });
        for it in &items {
            let a = detect_scene(&win, it, bbox, false, &t, false)
                .expect("windowed")
                .0;
            let b = detect_scene(&bulk, it, bbox, false, &t, false)
                .expect("bulk")
                .0;

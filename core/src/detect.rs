...
//! field of `Thresholds`, not a preset. `Thresholds::default()` is the validated
//! compact-source detector; shells can override its spectral fields à la carte.
...
/// counts are f64 to mirror js numeric comparison. the cli/wasm shells override
/// individual fields; everything unset keeps the original burnoff compact-source
/// gates below.
...
impl Default for Thresholds {
    /// Original burnoff/DAFI-derived compact-source gates. These were the L1C
    /// settings before the browser-only L2A compromise; importantly, the
    /// connected-component and peakedness gates reject hot roofs and solar fields
    /// rather than emitting every warm component as a "flare".
    fn default() -> Self {
        Thresholds {
            b12_min: 0.30,
            b11_min: 0.20,
            ...
            large_b12_min: 0.70,
            warm_fraction: 0.5,
            warm_max_pixels: 100.0,
            single_pixel_min: 0.65,
            max_cloud_local: 0.75,
            cloud_free_thresh: 0.30,
        }
    }
}
...
pub struct BlockMeta {
    ...
    /// Additive radiometric offset from the SAFE product metadata (normally
    /// -1000 for processing baseline N04+, zero before it).
    pub radiometric_offset: f64,
    /// QUANTIFICATION_VALUE from the SAFE metadata (normally 10000).
...
pub fn calibrate_dn(dn: f64, radiometric_offset: f64, quantification: f64) -> f64 {
    if dn == 0.0 {
        0.0
    } else {
        (dn + radiometric_offset) / quantification
    }
}

/// Collection-1/default calibration retained for existing wasm callers.
pub fn dn_to_reflectance(dn: f64) -> f64 {
    calibrate_dn(dn, -1000.0, 10000.0)
}
...
pub fn enumerate_blocks(
    ...
    let px0 = (((sw.0.max(img_min_x)) - img_min_x) / res_x)
        .floor()
        .max(0.0) as usize;
    let py0 = ((img_max_y - ne.1.min(img_max_y)) / res_y).floor().max(0.0) as usize;
    let px1 = (((ne.0.min(img_max_x)) - img_min_x) / res_x)
        .ceil()
        .min(img_width as f64) as usize;
    let py1 = ((img_max_y - sw.1.max(img_min_y)) / res_y)
        .ceil()
        .min(img_height as f64) as usize;
    if px1 <= px0 || py1 <= py0 {
        return Vec::new();
    }
...
pub fn screen_clouds(scl: &[u8], max_cloud_local: f64, cloud_free_thresh: f64) -> (bool, bool) {
    ...
    if frac > max_cloud_local {
        (true, false)
    } else {
        (false, frac <= cloud_free_thresh)
    }
}
...
pub fn label_connected_components(mask: &[u8], width: usize, height: usize) -> (Vec<i32>, i32) {
    ...
    for i in 0..mask.len() {
        ...
        while let Some(idx) = q.pop_front() {
            ...
            if r > 0 {
                visit(idx - width, &mut q, &mut labels);
            }
            if r < height - 1 {
                visit(idx + width, &mut q, &mut labels);
            }
            if c > 0 {
                visit(idx - 1, &mut q, &mut labels);
            }
            if c < width - 1 {
                visit(idx + 1, &mut q, &mut labels);
            }
...
fn hot_core(
    ...
    while let Some(idx) = q.pop_front() {
        size += 1;
        rad += (b12[idx] - config.background).max(0.0);
        if size > config.cap {
            return (size, rad);
        }
        let r = idx / config.width;
        let c = idx % config.width;
        let visit = |n: usize, q: &mut VecDeque<usize>, visited: &mut [bool]| {
            if !visited[n] && labels[n] == label_id && b12[n] > config.threshold {
                visited[n] = true;
                q.push_back(n);
            }
        };
        if r > 0 {
            visit(idx - config.width, &mut q, &mut visited);
        }
        if r < config.height - 1 {
            visit(idx + config.width, &mut q, &mut visited);
        }
        if c > 0 {
            visit(idx - 1, &mut q, &mut visited);
        }
        if c < config.width - 1 {
            visit(idx + 1, &mut q, &mut visited);
        }
...
pub fn detect_block(
    ...
    if bg_pixels.len() < 10 {
        return (Vec::new(), block_cloud_free);
    }
    ...
    for i in 0..n {
        ...
        b11[i] = b11v;
        let b12v = b12[i];
        if b12v <= t.b12_min || b11v <= t.b11_min {
            continue;
        }
        if b12v <= contrast_thresh {
            continue;
        }
        if has_b8a {
            let b8av = calibrate_dn(
                b8a_raw.unwrap()[i] as f64,
                meta.radiometric_offset,
                meta.quantification,
            );
            let denom = b11v + b8av;
            let nhiswnir = if denom > 0.01 {
                (b11v - b8av) / denom
            } else {
                0.0
            };
            if !(nhiswnir > 0.0 || b11v > t.saturation || b12v > t.saturation) {
                continue;
            }
    ...
    for label_id in 1..=count {
        ...
        if peak_b12 < t.peak_b12_min {
            continue;
        }
        if np > t.large_pixels && peak_b12 < t.large_b12_min {
            continue;
        }
        let avg_b12 = sum_b12 / np;
        if n_pixels > 1 && peak_b12 < t.peakedness_min * avg_b12 && avg_b12 < t.saturation {
            continue;
        }
        if n_pixels == 1 && peak_b12 < t.single_pixel_min {
            continue;
        }
        ...
        let (core_px, radiance) = hot_core(
            &b12,
            &labels,
            label_id,
            peak_idx,
            HotCoreConfig {
                threshold: hot_thresh,
                background: median_bg,
                width: w,
                height: h,
                cap: t.warm_max_pixels as usize,
            },
        );
        if core_px as f64 > t.warm_max_pixels {
            continue;
        }
